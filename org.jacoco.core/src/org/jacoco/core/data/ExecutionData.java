 ...
 * has to be taken about the probe data array of type <code>LineData[]</code>
...
public final class ExecutionData {
	 ...
	 *            probe data
	 */
	public ExecutionData(final long id, final String name, final LineData[] data) {
		this.id = id;
		this.name = name;
		this.data = data;
	}
	...
	public ExecutionData(final long id, final String name, final int dataLength) {
		this.id = id;
		this.name = name;
		this.data = new LineData[dataLength];
	}
	...
	public void merge(final ExecutionData other) {
		...
		final LineData[] otherData = other.getData();
		for (int i = 0; i < data.length; i++) {
			if (data[i] != null && otherData[i] != null) {
				data[i].merge(otherData[i]);
